...
export const sessionLogout = (req, res) => {
    try {
        ...
        if (!req?.cookies?.token) { // FIXME:
            console.error("Error: No hay token para eliminar");
            return res.status(404).json({ status: "error", message: "No hay token para eliminar" });
        } else {
        ...
        return res.status(500).json({status: error, message: "Error del servidor al intentar cerrar sesion"})
    }
};